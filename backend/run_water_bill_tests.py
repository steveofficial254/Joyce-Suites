...
Usage:
    ...
    python run_water_bill_tests.py --method test_water_bill_creation
"""
...
def run_tests(verbose=False, test_class=None, test_method=None):
    """Run water bill tests with specified options"""
    ...
    cmd = ['python', '-m', 'pytest', 'tests/test_water_bill.py']
    
    if verbose:
        cmd.append('-v')
    
    if test_class:
        cmd.append(f'tests/test_water_bill.py::{test_class}')
    
    if test_method:
        if test_class:
            cmd[-1] += f'::{test_method}'
        else:
            cmd.append(f'tests/test_water_bill.py::{test_method}')
    
    # Add coverage if available
    try:
        import pytest_cov
        cmd.extend(['--cov=models.water_bill', '--cov=routes.rent_deposit', '--cov=routes.admin_routes'])
    except ImportError:
    ...
    print("🧪 Running Water Bill System Tests")
    print("=" * 50)
...
def main():
    """Main function"""
    parser = argparse.ArgumentParser(description='Run water bill system tests')
